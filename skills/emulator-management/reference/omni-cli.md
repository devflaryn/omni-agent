...
| `play_roblox(account, place_id, …)` | `omni play <username> --place <id> [--dev --job --launch-data --user-id --timeout] --json` | `{ok, place_id, deeplink, booted, launched, first_boot, adb_port, vnc_port, session, kiosk}`. Failure: `no_place`, `no_token`, `boot_timeout`. |
...
| `ensure_frida_server` / `hide_root_from_app` | Magisk `su` → devkit `omni-fridad` / `omni-hide` (dev base) | host frida endpoint / what was applied, or a not-a-dev-base error. |
...
| not-a-dev-base / not-rooted error from `ensure_frida_server`/`hide_root_from_app` | Instance is production or its boot isn't Magisk-rooted. | Boot with `dev=true`; ensure `omni build-dev-base --patch-boot` produced a rooted devkit. |
| `the host 'frida' Python binding is not importable` | The host runtime has no `frida` module — `ensure_frida_server` (guest side) can succeed while every host-side hook still fails. | `pip install frida==17.15.4` (match the dev base's frida-server exactly). It's in the agent's requirements.txt; a fresh host needs it installed. |
