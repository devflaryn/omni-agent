...
- **You add accounts ONLY by `login_roblox_account` (cookie).** The browser `omni login` is a human step you cannot do — there is no other route for you to create an account.
- **Bases:** production (`base_arm`) for normal play; **dev** (`base_arm` + the `vdc` devkit disk = frida-server + Magisk root) for testing a new/swappable Roblox build and for apps with root/frida detection. A base is fixed at instance-create time.
...
| You have… | Call | Notes |
|---|---|---|
| a **cookie (file/string) + place id**, maybe a **new APK to test** | `launch_roblox_build(place_id, token_file=… \| token=…, apk_path=… , dev=…)` | THE default for "cookie + place (+ apk)". Does login → (if apk: decode→inject_session_bootstrap→recompile→sign→install) → play. `dev` defaults true when `apk_path` is given, false otherwise. Fails with a `stage` name; every stage is idempotent, so fix that stage and re-call. |
...
1. `launch_roblox_build(place_id=8737899170, token_file="cookie.txt")` — omit `apk_path` to log in and play whatever Roblox build is already installed; pass `apk_path="build.apk"` to build+install a new one first (that auto-selects the dev base).
...
The dev base gives you install-a-swappable-build + frida + Magisk root, and always-on screenshots.
1. `launch_roblox_build(place_id=…, token_file=…, apk_path="mybuild.apk")` — `dev` auto-true; this builds (with session bootstrap injected), installs, and plays as the cookie's account. For a pre-built/already-signed apk you still want tested by the login flow, this is the one call. It stages intermediates under `omni_build/<username>_decoded` (~400 MB) + `omni_build/<username>_build.apk` in the workspace and **auto-cleans them on success** — they remain ONLY when a stage failed (that's where you inspect a decode/inject/recompile failure), keyed by the `stage` in the error.
2. If the app has **root/frida detection**, before it renders: `ensure_frida_server(device_name=<username>)` → `hide_root_from_app("<target.package>", device_name=<username>)`. Both require a dev-base, Magisk-rooted boot (they return a clear error otherwise — the fix is `omni build-dev-base --patch-boot`). `ensure_frida_server` returns a `127.0.0.1:<port>` host endpoint to attach the frida toolset (`expand_tools("frida")` → `frida_trace` / `frida_run_script`). NOTE: the host needs the `frida` Python binding installed and version-matched to the dev base's frida-server (`pip install frida==17.15.4`) — `ensure_frida_server` starting cleanly does NOT prove the host can attach; the binding is separate. `frida_run_script(mode="attach", package_name="<pkg>")` attaches to the running app BY PACKAGE (resolved via the app list); use `mode="spawn"` to hook startup.
3. Verify by observation → load the `emulator-testing` skill for the record/analyze/report loop, or read the dev base's always-on frames with `read_auto_screenshots`.

## Instance lifecycle
- **Reuse vs reset:** the account-named instance is reused across calls. `ensure_emulator_running(reset=true)` (its default) discards the overlay and re-provisions a clean one; `reset=false` reuses the provisioned instance (~35 s cold boot vs minutes).
- **Two at once:** distinct account usernames / `session_name`s run concurrently — one per account.
- **Teardown:** QEMU VMs run **detached and outlive the agent process**, so `stop_emulator(device_name=<username>)` when done to free RAM/CPU. `stop_emulator(purge=true)` REMOVES the instance (deletes overlay + /data) so the next boot is completely fresh — destructive; use it to reclaim disk or force a clean rebuild, not between ordinary test runs.
- **RAM tiers:** `ensure_emulator_running(mode="playable"|"hard"|"brutal")` (4G/3G/2G) when the host is tight.
- **Never `omni remove`/delete an instance's files by hand** to clean up — power off with `stop_emulator`, and only `purge=true` when you truly mean to destroy it.

## Direct `omni` CLI — the deep reference
Every tool above wraps the frozen `omni …` contract. When you need a subcommand no tool exposes (e.g. `omni session --show`, `omni accounts --set-custom-name`, raw `omni logcat`/`omni screenshot`, `omni adb …`), or you're diagnosing an engine-level error, load `reference/omni-cli.md` with `read_skill_resource` — it lists the exact commands, flags, JSON shapes, and a failure→cause→fix table.
...
- **Retrying an expired cookie.** `bad_token` / `no_token` won't fix itself — you need a fresh cookie (a browser `omni login` is a human step; you can only refresh via a new cookie string/file).
- **`ensure_frida_server`/`hide_root_from_app` on a production or un-rooted instance.** They only work on a dev-base, `--patch-boot`-rooted account. Boot with `dev=true`.
