...
**Accounts are Roblox logins, not disks.** An "instance" is a THIN (~0.4 MB) COW overlay of the shared base, auto-created and named for the account. A saved Roblox account is just its cookie in `accounts.json`, keyed by username (added by the human via `omnidroid login`; the agent cannot log in — that needs a browser). For the Roblox flow, prefer `play_roblox(account="<username>", place_id=...)` — it boots a thin instance, resolves the cookie by username, and lands in-game with no menu/taps. `list_roblox_accounts()` shows which usernames are available. `install_apk_on_emulator` still works for testing a fresh Roblox build on the DEV base before it ships.
